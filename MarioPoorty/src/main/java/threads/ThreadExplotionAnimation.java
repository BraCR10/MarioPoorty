...
public class ThreadExplotionAnimation extends Thread{
   private  boolean isRunning = true;
    ...
    public void run() {
        while (isRunning) {       
            screen.setEnableExplotionFlag(false);
    ...
    private void simpleExplotion() {
        if (screen.getMatrizButtons()[sourceX][sourceY].getIcon() == screen.getMisteryBox()) {
            screen.getMatrizButtons()[sourceX][sourceY].setIcon(screen.getExplotion());
            ...
            for (int i = 0; i <= 1; i++) {
                for (int j = 0; j <= 1; j++) {
                    ...
                    if (posX >= 0 && posX < screen.getMatrizButtons().length &&
                        ...
                        screen.getMatrizButtons()[posX][posY].setIcon(screen.getExplotion());
                    }
                }
            }
